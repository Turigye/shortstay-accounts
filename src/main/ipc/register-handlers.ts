...
const defaultHandlers: IpcHandlers = {
  ...
  [IPC_CHANNELS.ASSET_CREATE]:()=>{throw new Error("Business session is unavailable");},[IPC_CHANNELS.ASSET_UPDATE]:()=>{throw new Error("Business session is unavailable");},
...
export function createBusinessIpcHandlers(
  ...
  return {
    ...
    [IPC_CHANNELS.ASSET_CREATE]:(payload)=>session.createAsset(payload),[IPC_CHANNELS.ASSET_UPDATE]:({id,...payload})=>session.updateAsset(id,payload),
