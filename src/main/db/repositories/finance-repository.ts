...
export interface LoanRecord {
  ...
  repaymentFrequency: RepaymentFrequency; installmentAmount: Ugx | null;
...
export interface LoanInput {
  ...
  principal: number; outstandingBalance: number; interestRateBasisPoints?: number;
  startDate: string; dueDate?: string | null; notes?: string | null;
  repaymentFrequency?: RepaymentFrequency; installmentAmount?: number | null;
  termMonths?: number | null;
}
...
export function createFinanceRepository(database: Database.Database, businessId: string) {
  ...
  function loans(): LoanRecord[] {
    return database.prepare<[string], any>("SELECT * FROM loans WHERE business_id=? AND archived_at IS NULL ORDER BY start_date DESC,id").all(businessId).map((row) => ({
      ...
      installmentAmount: row.installment_amount == null ? null : ugx(row.installment_amount),
  ...
  function monthlyReport(month:string):FinancialReport {
    ...
    const staffCash=database.prepare<[string,string],{payments:number;returns:number}>(`SELECT COALESCE(SUM(CASE WHEN direction='payment' THEN amount ELSE 0 END),0) payments,COALESCE(SUM(CASE WHEN direction='return' THEN amount ELSE 0 END),0) returns FROM staff_payments WHERE business_id=? AND substr(paid_at,1,7)=?`).get(businessId,month)??{payments:0,returns:0};
