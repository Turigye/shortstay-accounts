...
export function ExpenseEditor({units,accounts,suppliers,onSave,onCancel}:{units:BusinessUnit[];accounts:PaymentAccount[];suppliers:Supplier[];onSave:(value:IpcPayload<typeof IPC_CHANNELS.EXPENSE_CREATE>)=>Promise<void>|void;onCancel:()=>void}){
 const [purchaseType,setPurchaseType]=useState<"cash"|"credit">("cash"); const [scope,setScope]=useState<"unit"|"shared">("unit");
 ...
 return <form className="expense-editor" onSubmit={submit}>
  <div className="form-grid-two"><label className="field-group"><span>Date</span><input name="date" type="date" required defaultValue={new Date().toISOString().slice(0,10)}/></label><label className="field-group"><span>Amount</span><input name="amount" type="number" min="1" step="1" required/></label></div>
  <label className="field-group"><span>Category</span><select name="categoryId" required>{EXPENSE_CATEGORIES.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}</select></label>
  <div className="form-grid-two"><label className="field-group"><span>Scope</span><select value={scope} onChange={e=>setScope(e.target.value as typeof scope)}><option value="unit">Property unit</option><option value="shared">Shared</option></select></label>{scope==="unit"?<label className="field-group"><span>Unit</span><select name="unitId" required>{units.map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select></label>:null}</div>
  ...
  <label className="field-group"><span>Reference</span><input name="reference"/></label><label className="field-group"><span>Notes</span><textarea name="notes"/></label>
