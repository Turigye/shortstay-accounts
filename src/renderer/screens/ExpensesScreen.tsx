...
export function ExpensesScreen({ units }: { readonly units: BusinessUnit[] }) {
  ...
  return <section className="expenses-screen">
    ...
    {panel ? <aside className="expense-side-panel"><header><h2>{panel === "expense" ? "New expense" : panel === "supplier" ? "New supplier" : panel === "recurring" ? "New recurring bill" : "Supplier payment"}</h2><button className="icon-button" aria-label="Close panel" onClick={() => setPanel(null)} type="button"><X size={17}/></button></header>
