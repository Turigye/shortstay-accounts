...
export function ExpensesScreen({ units }: { readonly units: BusinessUnit[] }) {
  ...
  const [selectedRecurring, setSelectedRecurring] = useState<RecurringExpenseTemplate | null>(null);
  ...
  async function saveExpense(value: IpcPayload<typeof IPC_CHANNELS.EXPENSE_CREATE>) {
    ...
    if (selectedRecurring) {
      const advanceResult = await window.stayBooks.invoke(IPC_CHANNELS.RECURRING_EXPENSE_ADVANCE, { id: selectedRecurring.id });
      if (!advanceResult.ok) { setError(firstError(advanceResult)); throw new Error(advanceResult.message); }
      setRecurring((current) => current.filter(({ id }) => id !== selectedRecurring.id));
    }
    setExpenses((current) => [result.data, ...current]); setSelectedRecurring(null); setPanel(null);
  }
  ...
  return <section className="expenses-screen">
    ...
    <div className="expense-layout">
      <RecurringExpenseReview templates={recurring} onAdd={() => setPanel("recurring")} onRecord={(template) => { setSelectedRecurring(template); setPanel("expense"); }}/>
      <div className="statement-panel"><div className="table-scroll">{loading ? <div className="statement-loading">Loading expenses</div> : filtered.length ? <table className="statement-table expense-table"><thead><tr><th className="date-column">Date</th><th>Category</th><th>Scope</th><th>Supplier</th><th>Status</th><th className="money-column">Amount</th><th className="money-column">Due</th><th className="action-column"><span className="visually-hidden">Actions</span></th></tr></thead><tbody>{filtered.map((expense) => <tr key={expense.id}><td>{expense.date}</td><td><strong>{labels.get(expense.categoryId)}</strong></td><td>{expense.scope === "shared" ? "Shared" : units.find((item) => item.id === expense.unitId)?.name ?? "Unit"}</td><td>{expense.supplierName ?? "-"}</td><td><span className={`expense-status ${expense.paymentStatus}`}>{expense.paymentStatus}</span></td><td className="money-cell">{formatUgx(expense.amount)}</td><td className="money-cell">{formatUgx(expense.due)}</td><td className="action-column">{expense.due > 0 ? <button className="text-button" onClick={() => { setSelected(expense); setPanel("payment"); }} type="button">Pay</button> : null}</td></tr>)}</tbody></table> : <div className="inline-empty"><ReceiptText size={22}/><strong>No expenses for this view</strong></div>}</div></div>
    </div>
    {panel ? <aside className="expense-side-panel"><header><h2>{panel === "expense" ? selectedRecurring ? "Record recurring bill" : "New expense" : panel === "supplier" ? "New supplier" : panel === "recurring" ? "New recurring bill" : "Supplier payment"}</h2><button className="icon-button" aria-label="Close panel" onClick={() => { setPanel(null); setSelectedRecurring(null); }} type="button"><X size={17}/></button></header>
      {panel === "expense" ? <ExpenseEditor units={units} accounts={accounts} suppliers={suppliers} defaults={selectedRecurring ? { amount: selectedRecurring.expectedAmount ?? undefined, categoryId: selectedRecurring.categoryId, scope: selectedRecurring.scope, unitId: selectedRecurring.unitId, notes: selectedRecurring.notes } : undefined} onSave={saveExpense} onCancel={() => { setPanel(null); setSelectedRecurring(null); }}/> : panel === "supplier" ? <form className="expense-editor" onSubmit={saveSupplier}><label className="field-group"><span>Name</span><input name="name" required/></label><label className="field-group"><span>Phone</span><input name="phone"/></label><button className="primary-button">Add supplier</button></form> : panel === "recurring" ? <form className="expense-editor" onSubmit={saveRecurring}><p className="editor-intro">Create the schedule once. The bill will reappear when it needs confirmation.</p><label className="field-group"><span>Bill category</span><select name="categoryId">{EXPENSE_CATEGORIES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><label className="field-group"><span>Applies to</span><select name="scope"><option value="shared">Both units / business</option><option value="unit">One property unit</option></select></label><label className="field-group"><span>Unit when applicable</span><select name="unitId"><option value="">Choose unit</option>{units.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="field-group"><span>Expected amount</span><input name="expectedAmount" type="number" min="0" required/></label><label className="field-group"><span>Repeats</span><select name="cadence"><option value="monthly">Every month</option><option value="quarterly">Every three months</option><option value="annually">Every year</option></select></label><label className="field-group"><span>First confirmation month</span><input name="nextReviewMonth" type="month" defaultValue={month} required/></label><label className="field-group"><span>Notes</span><textarea name="notes"/></label><button className="primary-button">Save recurring bill</button></form> : <form className="expense-editor" onSubmit={paySupplier}><p>Remaining: <strong>{formatUgx(selected?.due ?? 0)}</strong></p><label className="field-group"><span>Amount</span><input name="amount" type="number" max={selected?.due} min="1" required/></label><label className="field-group"><span>Payment date</span><input name="paidAt" type="date" defaultValue={new Date().toISOString().slice(0, 10)} required/></label><label className="field-group"><span>Account</span><select name="accountId" required>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="field-group"><span>Reference</span><input name="reference"/></label><button className="primary-button">Record payment</button></form>}
    </aside> : null}
  </section>;
}
